...
def calc_charge(pka: float, pH: float = 7.0) -> float:
    ...
    ppos = 1.0 / (1 + 10 ** (pH - pka))  # fraction of more positively charged res
    return ppos
...
def calc_tmatrix(
    ...
    for idx, row in enumerate(tmatrix_mean):
        tmatrix[idx, idx] = np.prod(-1.0 * row + 1)  # probability not to transition
...
def reconstruct_free_energies_weighted(
    ...
    for _ in range(max_iter):
        ...
        G_reduced_new, *_ = np.linalg.lstsq(A_w_reduced, b_w, rcond=None)
