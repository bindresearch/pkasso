...
def register_model(name):
    """
    New model types can be added to unicoreinfer with the :func:`register_model`
    function decorator.
...
def register_model_architecture(model_name, arch_name):
    """
    New model architectures can be added to unicoreinfer with the
...
for file in os.listdir(models_dir):
    ...
    if not file.startswith("_") and not file.startswith(".") and (file.endswith(".py") or os.path.isdir(path)):
        ...
        module = importlib.import_module("unicoreinfer.models." + model_name)
