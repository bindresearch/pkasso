...
try:
    import unicoreinfer_fused_layernorm
    import unicoreinfer_fused_layernorm_backward_gamma_beta
    HAS_LAYER_NORM = True
except:
...
class FusedLayerNormFastFunction(torch.autograd.Function):
  ...
  def forward(ctx, input, weight, bias, normalized_shape, eps):
    ...
    output, mean, invvar = unicoreinfer_fused_layernorm.forward(
  ...
  def backward(ctx, grad_output):
    ...
    grad_input = unicoreinfer_fused_layernorm.backward(
        grad_output.contiguous(), mean, invvar,
        input_, ctx.normalized_shape,
        weight_, bias_, ctx.eps)
    grad_weight, grad_bias = unicoreinfer_fused_layernorm_backward_gamma_beta.backward(
        grad_output.contiguous(), mean, invvar,
        input_, ctx.normalized_shape,
        weight_, bias_, ctx.eps)
